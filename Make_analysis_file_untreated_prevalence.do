...
March 2023- changed it to alter way it assigns prevalence when the min and max of reported ages are the same
previously added 5 to max and took 5 from min to give a 10 year range. That seems too much so changed it to add/subtract 1 to give 3 year range
Aug 2023- added in interpolation by calendar year since this was missing, used to be there
************************************************************************/
...
**use the prevalence data
use "${alphapath}/ALPHA\Ready_data_untreated_prevalence/${sitename}/observed_proportion_positive_${sitename}",clear
cap drop _merge
	...
	foreach f in `flist'{
		** MEN
		forvalues x=15/59 {
			...
			local agemax=r(mean)
			*if both ages are the same, spread them out 1 years either side- i.e. a 3 year range
			if `agemin'==`agemax' {
				local agemin=`agemin'-1
				local agemax=`agemax'+1
				}
		...
		** WOMEN
		forvalues x=15/59 {
			...
			local agemax=r(mean)
			*if both min and max ages are the same, spread them out 1 years either side- i.e. a 3 year range
			if `agemin'==`agemax' {
				local agemin=`agemin'-1
				local agemax=`agemax'+1
				}
...
use "${alphapath}/ALPHA/Ready_data_untreated_prevalence/${sitename}/prevalence_treatment_in_opposite_sex_${sitename}", clear
drop opp_sex_prev opp_sex_prev_lb opp_sex_prev_ub opp_sex_treated
*INTERPOLATE FOR MISSING CALENDAR YEARS (added Aug 2023)
*should be 90 records per year- 2 sexes and 45 ages

*Find missing years and create a set of empty records for those years
summ years_one
...
di "`y'"
count if years_one==`y'
local n=r(N)
if `n'==0 {
expand 2 if years_one==`first_year',gen(added_set)
replace years_one=`y' if added_set==1
replace untreated_opp_sex_prevalence=. if added_set==1
drop added_set
}
}
...
bys sex age: ipolate untreated_opp_sex_prevalence years_one, gen(interpolated_untreated)
replace untreated=interp if untreated==.


*** extend at either end of the year range- so borrow from the first year for the year before and carry the last year onto one extra year
summ years_one if untreated<.
local minyr=r(min)
local beforeyr=`minyr'-1
local maxyr=r(max)
local afteryr=`maxyr'+1

bys sex age:egen max_un1=max(untreated) if years_one>=`beforeyr' & years_one<=`minyr'
replace untreated=max_un1 if years_one==`beforeyr'

bys sex age:egen max_un2=max(untreated) if years_one>=`maxyr' & years_one<=`afteryr'
replace untreated=max_un2 if years_one==`afteryr'

drop max_un*

save "${alphapath}/ALPHA/Ready_data_untreated_prevalence/${sitename}/interpolated_untreated_in_opposite_sex_${sitename}",replace
