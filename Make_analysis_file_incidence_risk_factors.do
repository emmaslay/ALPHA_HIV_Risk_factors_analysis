*updated April 2023 to add in education status
*updated Aug 2023 to use interpolated untreated prev estimates 
********** PREPARE THE IMPUTATIONS
global sitelist="Ifakara Karonga Kisesa Kisumu Manicaland Masaka Rakai uMkhanyakude"
*global sitelist=" Masaka Rakai uMkhanyakude"
...
*global sitename="kisesa"
	...
	**** BRING IN EDUCATION (derived from spec 7.4)
	cap drop _merge
	merge  m:1 study_name idno using  "${alphapath}/ALPHA/prepared_data/${sitename}/educ_wide2_${sitename}",gen(educ_merge)
	drop if educ_merge==2
	gen tv_educ=.
	label var tv_educ "Time varying education status (highest completed level)"

	** SPLIT WHEN EDUCATION	STATUS CHANGES
	stsplit ed0 if start_educset_date0<.,after(start_educset_date0) at(0)
	stsplit ed1 if start_educset_date1<.,after(start_educset_date1) at(0)
	stsplit ed2 if start_educset_date2<.,after(start_educset_date2) at(0)
	stsplit ed3 if start_educset_date3<.,after(start_educset_date3) at(0)
	** ASSIGN EDUCATION	STATUS TO EACH EPISODE
	replace tv_educ=0 if ed0==0 
	replace tv_educ=1 if ed1==0 
	replace tv_educ=2 if ed2==0 
	replace tv_educ=3 if ed3==0 
	*make this missing if all transitions are missing data or if the episode is before the first report of education status
	replace tv_educ=9 if ed0~=0 & ed1~=0 & ed2~=0 & ed3~=0 

	label define tv_educ 0 "No education" 1 "Primary" 2 "Secondary" 3 "Tertiary" 9 "Unknown",modify

	label values tv_educ tv_educ

	******** HARMONISE RESIDENCE
	label define residence 1 "Rural" 2 "Urban" 3 "Peri-urban" 4 "<1km from road" 5 ">1km from road" 6 "agricultural estate" 7	"commercial center" 8	"roadside trading" 9	"subsistence farming",modify
	*Karonga
	replace residence=residence+4 if study_name==1
	*Manicaland
	replace residence=residence+5 if study_name==3
	*Kisesa
	recode residence 0=1 1=3 if study_name==2
	*Rakai
	replace residence=1 if study_name==5
	*uMkhanyakude- guessing as not labelled so need to check
	recode residence 0=2 1=3 2=1 if study_name==6
	*Kisumu- residency is Gem, Asembo, Karemo but think all of Gem is rural
	replace residence=1 if study_name==8
	*Ifakara, all urban
	replace residence=2 if study_name==9
	...
	* 	BRING IN PREVALANCE IN OPPOSITE SEX
	cap drop _merge
	merge m:1 study_name sex age years_one using ${alphapath}/ALPHA/Ready_data_untreated_prevalence/${sitename}/interpolated_untreated_in_opposite_sex_${sitename}
	drop if _m==2
	drop _merge
...
*if the lowest value is above 88 then that is missing/not asked etc so set to missing
replace age_at_circ_lowest=. if age_at_circ_lowest>=88 & sex==1
	...
	*put not applicable/missing into 0. must control for num_ptnrs
	...
	recode tv_partners_lastyear (min/-1=.) (0=0) (1=1) (2=2) (3/90=3 "3+") (100/200=3) (97=9 "Don't know"),gen(tv_ptnrs) label(tv_ptnrs)
	*collapse number of partners categories
	label define tv_ptnrs 0 "No partners" 1 "1 partner" 2 "2 partners" 3 "3+ partners" 9 "Don't know",modify
	label var tv_ptnrs "Number of partners in last year, grouped"

** FIX TV_ANYLOST FOR MASAKA- SHOULD ALL BE DON'T KNOW AS DIDN'T COLLECT THAT INFO
	replace tv_anylost=9 if study_name==4
...
*** MARITAL STATUS
	...
	*pull in the ever married status from sexual behaviour spec if it is missing
	...
	replace tv_mstat_br=3 if tv_marever==1 & tv_mstat_br==.
	replace tv_mstat_br=9 if tv_mstat_br==.
	label define tv_mstat_br 3 "Ever married no current info",modify
